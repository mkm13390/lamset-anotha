<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>الموافقات</title>
<style>
...
<div class="top"><div><h1>طلبات الموافقات</h1><div class="muted">اعتماد العمليات الحساسة قبل تنفيذها</div></div>
<div class="nav">
...
<div class="card"><form method="get" class="inline"><div class="field"><label>الحالة</label><select name="status"><option value="">الكل</option><option value="pending">pending</option><option value="approved">approved</option><option value="rejected">rejected</option><option value="cancelled">cancelled</option></select></div><button class="primary">فلترة</button></form></div>
<div class="card"><table><thead><tr><th>الرقم</th><th>النوع</th><th>الطالب</th><th>السبب</th><th>الحالة</th><th>التاريخ</th><th>قرار</th></tr></thead><tbody>
@forelse($requests as $r)<tr><td>{{ $r->request_number }}</td><td>{{ $r->approval_type }}</td><td>{{ $r->requester?->name }}</td><td>{{ $r->reason ?: '-' }}</td><td><span class="badge">{{ $r->status }}</span></td><td>{{ optional($r->requested_at)->format('Y-m-d H:i') }}</td><td>
@if($r->status==='pending')<form method="post" action="{{ route('admin.approvals.review',$r) }}">@csrf<div class="inline"><select name="decision"><option value="approved">اعتماد</option><option value="rejected">رفض</option></select><input name="review_notes" placeholder="ملاحظة"><button>تنفيذ</button></div></form>@else {{ $r->reviewer?->name ?: '-' }} @endif
</td></tr>@empty<tr><td colspan="7" class="empty">لا توجد طلبات موافقة.</td></tr>@endforelse
</tbody></table><div>{{ $requests->links() }}</div></div>
</div></body></html>